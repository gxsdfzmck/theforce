drop table if exists `zhyscore_16`;

CREATE TABLE `zhyscore_16` (
...
/*处理院校录取数据*/
insert into zhyscore_16(uname, subject, zhyname)
 select distinct university_name, subject_name, major_name
 from raw_data.sdb_enrollment_file_score_16
 where is_major=1;

/*按年份处理录取数据*/
update zhyscore_16 u, raw_data.sdb_enrollment_file_score_16 s
 set u.enroll2015=s.plan_number, u.ascore2015=s.min_score, u.abatch2015=s.batching_name
 where s.year=2015 and u.subject=s.subject_name and u.uname=s.university_name and u.zhyname=s.major_name;

update zhyscore_16 u, raw_data.sdb_enrollment_file_score_16 s
 set u.enroll2014=s.plan_number, u.ascore2014=s.min_score, u.abatch2014=s.batching_name
 where s.year=2014 and u.subject=s.subject_name and u.uname=s.university_name and u.zhyname=s.major_name;

update zhyscore_16 u, raw_data.sdb_enrollment_file_score_16 s
 set u.enroll2013=s.plan_number, u.ascore2013=s.min_score, u.abatch2013=s.batching_name
 where s.year=2013 and u.subject=s.subject_name and u.uname=s.university_name and u.zhyname=s.major_name;

update zhyscore_16 u, dev_sdb.sdb_leveled_file_ranking2 r
...
/*处理学校相关信息数据*/
update zhyscore_16 s, sdb.sdb_university u
set s.uid=u.id
...
update zhyscore_16 s, sdb.sdb_university u
 ...
 where s.uid=u.id;
update zhyscore_16 s, sdb.sdb_university_info i 
 set s.city=i.city
 where s.uid=i.uid;
update zhyscore_16 s, sdb.sdb_province p
 set s.pname=p.name
 where s.pid=p.id;
update zhyscore_16 s, sdb.sdb_xyh_ranking r, sdb.sdb_university u
 set s.utyperank=r.utype_ranking
 where u.name=r.university_name and u.id=s.uid;

/* TODO 处理专业信息 */
update zhyscore_16 s, sdb.sdb_major m
  set s.zhyid=m.id
