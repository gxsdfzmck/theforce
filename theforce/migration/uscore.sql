drop table if exists `uniscore_7`;

CREATE TABLE `uniscore_7` (
...
/*处理院校录取数据*/
insert into uniscore_7(uname, subject)
 select distinct university_name, '文科'
 from rd2016.u_score_7;
insert into uniscore_7(uname, subject)
 select distinct university_name, '理科'
 from rd2016.u_score_7;

/*按年份处理录取数据*/
update uniscore_7 u, rd2016.u_score_7 s
...
update uniscore_7 u, dev_sdb.sdb_leveled_file_ranking2 r
...
update uniscore_7 s, dev_sdb.sdb_university u
set s.uid=u.id
where s.uname = u.name;

insert into uniscore_7(uid, uname, subject, belong) /*belong为标记是否新增*/
 select id, name, '文科', '-'
 from dev_sdb.sdb_university
 where id not in (select uid from uniscore_7);
insert into uniscore_7(uid, uname, subject, belong)
 select uid, uname, '理科', '-'
 from uniscore_7
 where belong = '-';
